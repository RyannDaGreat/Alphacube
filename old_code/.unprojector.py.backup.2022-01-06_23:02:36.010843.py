...
def unproject_translations_v2(scene_uvs, scene_labels, scene_translations, num_labels:int, output_height:int, output_width:int,version='slow'):
    ...
    if version=='fast':
        ...
        #     output_weight[l, u, v] += w 
        #  #TODO: Make this a more generalizable function

        output_sum=output_sum.view(NL*OH*OW*NC)
        output_sum.index_add_(0,l*OH*OW*NC + u*OW*NC + v*NC + 0,t[:,0]*w)
        output_sum.index_add_(0,l*OH*OW*NC + u*OW*NC + v*NC + 1,t[:,1]*w)
        output_sum.index_add_(0,l*OH*OW*NC + u*OW*NC + v*NC + 2,t[:,2]*w)
        output_sum=output_sum.view(NL,OH,OW,NC)
...
def test_unproject_translations():
    ...
    print('FAST VERSION OUTPUT (should be same as SLOW):')
    icecream.ic(out_tex[0,0],out_weight[0,0])
