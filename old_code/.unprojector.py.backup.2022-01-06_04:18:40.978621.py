...
def unproject_translations(scene_uvs, scene_labels, scene_translations, num_labels:int, output_height:int, output_width:int):
    ...
    assert scene_labels.shape == (batch_size, output_height, output_width)
    ...
    l = scene_labels 
    l = einops.repeat(l, 'BS ')

    t = einops.rearrange(t, 'ABCD BS NC SH SW -> BS (SH SW ABCD) NC')
    u = einops.rearrange(u, 'ABCD BS    SH SW -> BS (SH SW ABCD)  1')
    v = einops.rearrange(v, 'ABCD BS    SH SW -> BS (SH SW ABCD)  1')

    output_sum = einops.rearrange(output_sum, 'NL NC OH OW -> NL OH OW NC')

    for i in range(batch_size):
        ##FUUUCKK I NEED TO TAKE INTO ACOUNT LABELS TOOO!!!!! GODDAMN IT....
        #output[label, uv indices]+=image pixels
