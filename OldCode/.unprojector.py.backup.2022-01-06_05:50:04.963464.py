...
def unproject_translations(scene_uvs, scene_labels, scene_translations, num_labels:int, output_height:int, output_width:int):
    ...
    output_sum    = torch.zeros((num_labels, output_height, output_width, num_channels), 
                                requires_grad = False                                  ,
...
def test_unproject_translations():
    #Right now this is just a test to make sure it doesn't crash.
    #It doesn't test for correctness yet.
    #def unproject_translations(scene_uvs, scene_labels, scene_translations, num_labels:int, output_height:int, output_width:int)
    C = 3
    L = 5
    B = 3
